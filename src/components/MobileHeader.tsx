      ...
      <div className="flex h-14 items-center justify-between px-4">
        <div className="flex items-center gap-3">
          {onBack ? <Button variant="ghost" size="sm" onClick={onBack} className="h-8 w-8 p-0">
            ...
            </Button> : activeTab === 'home' ? <div className="cursor-pointer" onClick={() => onTabChange('home')}>
              <GrubbyLogo size="sm" />
            </div> : null}
        ...
        {showSettings && <div className="flex items-center">
            {user ? <Button variant="ghost" size="sm" onClick={() => onTabChange('settings')} className="h-8 w-8 p-0">
                <Settings className="h-4 w-4" />
              </Button> : <Button variant="outline" size="sm" onClick={() => navigate('/auth')} className="text-xs">
                Sign In
              </Button>}
          </div>}
      </div>
