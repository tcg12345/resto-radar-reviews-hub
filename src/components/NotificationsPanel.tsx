...
export function NotificationsPanel() {
  ...
  useEffect(() => {
    ...
    const subscription = supabase.channel('notifications-changes').on('postgres_changes', {
      event: 'INSERT',
      schema: 'public',
      table: 'notifications',
  ...
  const handleAddToWishlist = async (notificationId: string, restaurantId: string) => {
    try {
      ...
      const {
        error: shareError
      } = await supabase.from('restaurant_shares').update({
        status: 'added_to_wishlist'
      }).eq('restaurant_id', restaurantId).eq('receiver_id', user?.id);
  ...
  return <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button variant="ghost" size="icon" className="relative">
          ...
          {unreadCount > 0 && <span className="absolute top-0 right-0 flex h-4 w-4 items-center justify-center rounded-full bg-primary text-[10px] font-bold text-primary-foreground">
      ...
      <PopoverContent align="end" className="w-80 p-0 max-h-[500px] overflow-hidden flex flex-col">
        <div className="flex items-center justify-between p-4">
          ...
          {unreadCount > 0 && <Button variant="ghost" size="sm" className="h-8 text-xs" onClick={markAllAsRead}>
        ...
        <div className="flex-1 overflow-y-auto">
          {loading ? <div className="p-4 text-center">
              ...
              {notifications.map(notification => <div key={notification.id} className={`relative p-4 border-l-4 ${!notification.read_at ? 'border-l-primary bg-primary/5' : 'border-l-muted bg-background'} ${notification.type === 'restaurant_share' ? 'cursor-pointer hover:bg-muted/30 transition-all duration-200' : ''}`} onClick={() => notification.type === 'restaurant_share' && handleViewRestaurant(notification.id, notification.data?.restaurant_id)}>
                  {/* Unread indicator */}
                  {!notification.read_at && <div className="absolute top-2 right-2 w-2 h-2 bg-primary rounded-full"></div>}
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0 space-y-2 -mx-1">
                      ...
                      {/* Restaurant card */}
                      {notification.type === 'restaurant_share' && <div className="bg-background rounded-lg border p-3 shadow-sm w-full">
                          <div className="flex items-center justify-between">
                            ...
                            <Button variant="outline" size="sm" className="h-8 text-xs px-3 ml-3 pointer-events-none shrink-0">
                      ...
                      <div className="flex justify-between items-center pt-1">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          {notification.data?.share_message && <div className="flex items-center gap-2 p-2 rounded-lg border border-border bg-background/50">
                              <Button variant="outline" size="sm" className="h-7 px-2 text-xs border-primary/20 bg-primary/5 hover:bg-primary/10 text-primary font-medium" onClick={e => {
                        e.stopPropagation();
                        showFullMessage(notification.data.share_message);
                              ...
                              <Button variant="outline" size="sm" className="h-7 w-7 p-0 border-destructive/20 bg-destructive/5 hover:bg-destructive/10 text-destructive" onClick={e => {
                        e.stopPropagation();
                        ...
                        deleteNotification(notification.id);
                              ...
                              {!notification.read_at && <Button variant="outline" size="sm" className="h-7 w-7 p-0 border-primary/20 bg-primary/5 hover:bg-primary/10 text-primary" onClick={e => {
                        e.stopPropagation();
