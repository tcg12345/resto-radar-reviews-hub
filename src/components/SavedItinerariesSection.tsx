...
export function SavedItinerariesSection({
  onLoadItinerary
}: SavedItinerariesSectionProps) {
  const [filter, setFilter] = useState<ItineraryFilter>('all');
  const navigate = useNavigate();
  const {
    itineraries: savedItineraries,
    loading,
    deleteItinerary: removeItinerary,
    refetch
  } = useItineraries();
  ...
  const exportItinerary = (itinerary: Itinerary) => {
    ...
    try {
      ...
      if (itinerary.events.length > 0) {
        ...
        Object.entries(eventsByDate).sort(([a], [b]) => a.localeCompare(b)).forEach(([date, events], dayIndex) => {
          checkNewPage(30);
          ...
          events.sort((a, b) => a.time.localeCompare(b.time)).forEach((event, eventIndex) => {
            checkNewPage(25);

            // Event time with icon based on type
            doc.setFontSize(12);
            doc.setFont('helvetica', 'bold');
            const eventIcon = event.type === 'restaurant' ? '[Restaurant]' : event.type === 'attraction' ? '[Attraction]' : '[Event]';
            ...
            if (event.restaurantData) {
              ...
              if (event.restaurantData.website) {
                const websiteText = event.restaurantData.website.length > 50 ? event.restaurantData.website.substring(0, 47) + '...' : event.restaurantData.website;
            ...
            if (event.attractionData) {
              ...
              if (event.attractionData.website) {
                const websiteText = event.attractionData.website.length > 50 ? event.attractionData.website.substring(0, 47) + '...' : event.attractionData.website;
                doc.text(`Website: ${websiteText}`, margin + 15, yPosition);
                yPosition += 4;
              }
            }
  ...
  return <div className="space-y-6">
      ...
      <Tabs value={filter} onValueChange={value => setFilter(value as ItineraryFilter)} className="w-full">
        ...
        <TabsContent value={filter} className="mt-6">
          {filteredItineraries.length === 0 ? <div className="text-center py-8">
              ...
              {filteredItineraries.map(itinerary => <Card key={itinerary.id} className="transition-all duration-200 hover:shadow-md my-[12px]">
                  {/* Desktop Layout */}
                    ...
                    <CardHeader>
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          <CardDescription className="space-y-2">
                            <div className="flex items-center gap-4 text-sm">
                              ...
                              {itinerary.isMultiCity && <Badge variant="outline">Multi-city</Badge>}
                            </div>
                            <div className="flex items-center gap-2 flex-wrap">
                              {itinerary.locations.map((location, index) => <Badge key={location.id} variant="secondary" className="flex items-center gap-1">
                        ...
                        <div className="flex items-center gap-2 ml-4">
                          <Button variant="outline" size="sm" onClick={() => navigate(`/itinerary/${itinerary.id}`)} className="flex items-center gap-2">
                            <Eye className="w-4 h-4" />
                            View
                          </Button>
                          <Button variant="outline" size="sm" onClick={() => onLoadItinerary(itinerary)} className="flex items-center gap-2">
                            <Edit className="w-4 h-4" />
                            Edit
                          </Button>
                          <Button variant="outline" size="sm" onClick={() => exportItinerary(itinerary)} className="flex items-center gap-2">
                          ...
                          <AlertDialog>
                            ...
                            <AlertDialogContent>
                              ...
                              <AlertDialogFooter>
                                ...
                                <AlertDialogAction onClick={() => deleteItinerary(itinerary.id!)} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">
                                  Delete
                                </AlertDialogAction>
                              </AlertDialogFooter>
                            </AlertDialogContent>
                    ...
                    <CardHeader className="pb-3">
                      <div className="space-y-3">
                        ...
                        <div className="flex gap-1.5 overflow-x-auto pb-1 scrollbar-hide">
                          {itinerary.locations.map(location => <Badge key={location.id} variant="secondary" className="flex items-center gap-1 text-xs whitespace-nowrap flex-shrink-0">
                              <MapPin className="w-2.5 h-2.5" />
                              {location.name}
                            </Badge>)}
                          {itinerary.isMultiCity && <Badge variant="outline" className="text-xs whitespace-nowrap flex-shrink-0">Multi-city</Badge>}
                        </div>

                        {/* Action Buttons - Stacked on mobile */}
                        <div className="flex flex-col gap-2">
                          <div className="flex gap-2">
                            <Button variant="outline" size="sm" onClick={() => navigate(`/itinerary/${itinerary.id}`)} className="flex-1 h-8 text-xs">
                              <Eye className="w-3 h-3 mr-1" />
                              View
                            </Button>
                            <Button variant="outline" size="sm" onClick={() => onLoadItinerary(itinerary)} className="flex-1 h-8 text-xs">
                              <Edit className="w-3 h-3 mr-1" />
                              Edit
                            </Button>
                          </div>
                          <Button variant="outline" size="sm" onClick={() => exportItinerary(itinerary)} className="w-full h-8 text-xs">
                            <Download className="w-3 h-3 mr-1" />
                            Export
                          </Button>
                          <AlertDialog>
                            <AlertDialogTrigger asChild>
                              <Button variant="outline" size="sm" className="w-full h-8 text-xs text-destructive hover:text-destructive border-destructive/20">
                  ...
                  {itinerary.events.length > 0 && <CardContent className="pt-0">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="hidden md:block">
                          ...
                          <div className="grid gap-2">
                            {itinerary.events.slice(0, 3).map(event => <div key={event.id} className="flex items-center gap-2 text-sm text-muted-foreground">
                            ...
                            {itinerary.events.length > 3 && <div className="text-sm text-muted-foreground">
                        ...
                        <div className="md:hidden">
                          ...
                          <div className="flex gap-1 flex-wrap">
                            {itinerary.events.slice(0, 2).map(event => <Badge key={event.id} variant="outline" className="text-xs">
                                {event.type}
                              </Badge>)}
                            {itinerary.events.length > 2 && <Badge variant="secondary" className="text-xs">
