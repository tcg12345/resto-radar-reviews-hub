...
export default function ExpertRatingsPage() {
  ...
  useEffect(() => {
    document.title = `${name} • Expert Ratings`;
  }, [name]);
  ...
  useEffect(() => {
    const run = async () => {
      if (!placeId) return;
      setLoading(true);
      const { data, error } = await supabase.rpc('get_expert_reviews_for_place', {
      ...
      <div className="sticky top-0 z-50 bg-background backdrop-blur border-b pt-safe-area-top">
        <div className="flex items-center gap-3 p-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-lg font-semibold">Expert Ratings</h1>
          </div>
        </div>
      </div>

      <div className="p-4 space-y-3">
        {loading && <p className="text-sm text-muted-foreground">Loading expert ratings...</p>}
        {!loading && reviews.length === 0 && (
          <p className="text-sm text-muted-foreground">No experts have rated this place yet.</p>
        )}
