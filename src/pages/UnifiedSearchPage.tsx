  ...
  const performLiveSearch = async () => {
    ...
    try {
      ...
      const {
        data: aiData,
        error: aiError
      } = await supabase.functions.invoke('restaurant-discovery', {
        body: {
          query: fallbackQuery,
      ...
      if (!aiError && aiData?.restaurants?.length) {
        const mapped = aiData.restaurants.map((r: any) => ({
          ...
          opening_hours: typeof r.isOpen === 'boolean' ? {
            open_now: r.isOpen
          } : undefined,
        ...
        <div className="w-full py-4">
          ...
          <div className="space-y-4">
            <div className="w-full">
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 items-center mx-[8px]">
                {/* Main Search Input */}
                <div className="lg:col-span-2 relative" ref={searchRef}>
                  <div className="relative group w-full">
                    <div className="relative bg-background border border-border rounded-md hover:border-primary/50 transition-all duration-300 shadow-sm hover:shadow-md">
                      ...
                      <Input placeholder="Search restaurants, cuisines, or dishes..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-12 pr-10 h-10 bg-transparent border-none text-sm placeholder:text-muted-foreground/70 focus:ring-0 focus:outline-none rounded-md" />
                      {searchQuery && <button onClick={clearSearch} className="absolute right-2 top-1/2 transform -translate-y-1/2 text-muted-foreground hover:text-foreground transition-colors p-1 rounded-md hover:bg-muted/50">
                          <X className="h-3 w-3" />
                        </button>}
                    </div>
                  </div>
                ...
                <div className="relative">
                  <div className="relative group w-full">
                    <div className="relative bg-background border border-border rounded-md hover:border-primary/50 transition-all duration-300 shadow-sm hover:shadow-md">
                      ...
                      <Input placeholder="Location" value={locationQuery} onChange={e => {
                    ...
                    }} onFocus={() => locationQuery.length > 1 && setShowLocationSuggestions(true)} onBlur={() => setTimeout(() => setShowLocationSuggestions(false), 150)} className="pl-12 pr-3 h-10 bg-transparent border-none text-sm placeholder:text-muted-foreground/70 focus:ring-0 focus:outline-none rounded-md" />
                      
                      {/* Modern Location Suggestions */}
                      {showLocationSuggestions && locationSuggestions.length > 0 && <div className="absolute top-full left-0 right-0 z-50 mt-2 bg-card/95 backdrop-blur-lg border border-border rounded-lg shadow-2xl overflow-hidden animate-fade-in">
          ...
          {searchError && <Alert variant="destructive" className="mt-3">
      ...
      {!searchQuery && user && (searchResults.length === 0 || !isLoading) && <div className="lg:hidden mt-8 space-y-8">
          ...
          {recentClickedRestaurants.length > 0 && <div className="space-y-4">
              ...
              <div className="space-y-2">
                {recentClickedRestaurants.map(place => <Card key={place.place_id} className="overflow-hidden bg-card border-0 shadow-[0_4px_20px_rgba(0,0,0,0.08)] hover:shadow-[0_8px_30px_rgba(0,0,0,0.12)] transition-all duration-300 rounded-2xl cursor-pointer group" onClick={() => handlePlaceClick(place)}>
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <div className="flex items-center gap-2 flex-shrink-0">
                            ...
                            <Button size="sm" className="h-6 px-2.5 rounded-full bg-primary hover:bg-primary/90 text-primary-foreground font-medium text-xs" onClick={e => {
                      e.stopPropagation();
          ...
          {recommendedPlaces.length > 0 && <div className="space-y-4">
              <div className="flex items-center justify-between mb-1">
                <div className="flex items-center gap-3">
                  <h3 className="text-xl font-bold bg-gradient-to-r from-foreground to-foreground/80 bg-clip-text text-transparent ml-2">Recommended for you</h3>
                  <div className="h-0.5 w-8 bg-gradient-to-r from-primary to-primary-glow rounded-full"></div>
                </div>
              </div>
              
              {isLoadingRecommendations ? <div className="space-y-2">
                  {[...Array(3)].map((_, i) => <Card key={i} className="overflow-hidden bg-card border-0 shadow-[0_4px_20px_rgba(0,0,0,0.08)] rounded-2xl">
                      <CardContent className="py-2.5 px-3">
                        <div className="space-y-1">
                  ...
                  {recommendedPlaces.map((place, index) => <div key={place.place_id}>
          ...
          <TabsContent value="list" className="space-y-4">
            {isLoading ? <div className="space-y-3">
                ...
                {searchResults.map(place => <Card key={place.place_id} className="overflow-hidden bg-card border-0 shadow-[0_6px_25px_rgba(0,0,0,0.08)] hover:shadow-[0_12px_40px_rgba(0,0,0,0.15)] transition-all duration-300 rounded-2xl cursor-pointer group" onClick={() => handlePlaceClick(place)}>
                    <CardContent className="p-3 ml-2 rounded-l-xl" style={{
              backgroundColor: 'rgb(10,23,43)'
                      ...
                      <div className="lg:hidden">
                        ...
                        const cuisineTypes = place.types?.filter(type => !['restaurant', 'food', 'establishment', 'point_of_interest', 'store'].includes(type));
                      ...
                      <div className="hidden lg:block">
                        ...
                        return place.fallbackCuisine || 'Restaurant';
                          ...
                          return parts[0] || '';
                            ...
                            <div className="flex items-center gap-2">
                              <Button variant="outline" size="sm" className="h-7 px-3 rounded-full border-border hover:bg-muted/50 font-medium text-xs" onClick={() => handlePlaceClick(place)}>
                                View
                              </Button>
                              <Button size="sm" className="h-7 px-3 rounded-full bg-primary hover:bg-primary/90 text-primary-foreground font-medium text-xs" onClick={e => {
                        e.stopPropagation();
                        handleQuickAdd(place);
                      }}>
                                Add
