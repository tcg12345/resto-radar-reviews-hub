-- Improve friend/expert review RPCs to include name-based fallback when google_place_id is missing
...
BEGIN
  ...
  combined_reviews AS (
    -- User reviews from user_reviews table (always match by place_id)
    SELECT 
    ...
    WHERE r.rating IS NOT NULL 
      ...
      AND (
        ...
        OR 
        -- Fallback: if no google_place_id, match by restaurant name against any user_review for this place
...
-- Apply same fallback to expert reviews
...
BEGIN
  ...
  combined_reviews AS (
    ...
    WHERE r.rating IS NOT NULL 
      ...
      AND (
        r.google_place_id = place_id_param 
        OR (r.google_place_id IS NULL AND EXISTS (
          SELECT 1 FROM public.user_reviews ur2 
          WHERE ur2.restaurant_place_id = place_id_param
            AND (
              LOWER(TRIM(ur2.restaurant_name)) = LOWER(TRIM(r.name))
