-- Make community rating function work for all restaurants dynamically
...
BEGIN
  ...
  WITH matched_ratings AS (
    ...
    -- Strategy 2: Match by restaurant name if we have it from user_reviews
    SELECT 
      r.rating::double precision as rating,
      r.user_id,
      r.photos,
    ...
    WHERE r.rating IS NOT NULL 
      AND r.is_wishlist = false
      AND restaurant_name_from_reviews IS NOT NULL
      AND LOWER(r.name) = LOWER(restaurant_name_from_reviews)
