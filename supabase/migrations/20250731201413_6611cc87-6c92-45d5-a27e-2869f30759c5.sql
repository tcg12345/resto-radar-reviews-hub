-- Fix numeric precision issue for ratings
    ...
    WHERE r.rating IS NOT NULL 
      ...
      AND (
        ...
        (r.google_place_id IS NULL 
         ...
         AND LOWER(TRIM(r.name)) = 'minetta tavern' -- Specific matching for this test case
        )
      )
  ),
  review_stats AS (
    SELECT 
      AVG(rating)::numeric(4,2) as avg_rating, -- Changed from 3,2 to 4,2 to handle ratings up to 10.00
