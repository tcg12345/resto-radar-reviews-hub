-- Update get_restaurant_community_stats to include photos from personal ratings with privacy controls
...
BEGIN
  ...
  WITH combined_reviews AS (
    -- User reviews from user_reviews table (for ratings)
    SELECT 
...
-- Update get_restaurant_reviews to include personal ratings with privacy controls for photos
CREATE OR REPLACE FUNCTION public.get_restaurant_reviews(place_id_param text, page_limit integer DEFAULT 10, page_offset integer DEFAULT 0, sort_by text DEFAULT 'recent'::text, requesting_user_id uuid DEFAULT auth.uid())
...
BEGIN
  ...
  RETURN QUERY EXECUTE format('
    WITH combined_reviews AS (
      -- User reviews from user_reviews table (always visible)
      SELECT 
        ur.id,
        ur.user_id,
        ur.overall_rating,
      ...
      -- Personal restaurant ratings (photos only visible if public/friend)
      SELECT 
        r.id,
        r.user_id,
        ...
        CASE 
          WHEN p.is_public = true 
            ...
            OR ($4 IS NOT NULL AND EXISTS (
              SELECT 1 FROM public.friends f
              WHERE (f.user1_id = $4 AND f.user2_id = r.user_id)
                 OR (f.user2_id = $4 AND f.user1_id = r.user_id)
            ))
          THEN r.photos
          ELSE ARRAY[]::text[]
        END as photos,
        CASE 
          ...
          THEN r.photo_captions
          ELSE ARRAY[]::text[]
        END as photo_captions,
        CASE 
          WHEN p.is_public = true 
            OR r.user_id = $4
            OR ($4 IS NOT NULL AND EXISTS (
          ...
          THEN r.photo_dish_names
          ELSE ARRAY[]::text[]
        END as photo_dish_names,
        0 as helpful_count,
        r.created_at
      FROM public.restaurants r
      LEFT JOIN public.profiles p ON p.id = r.user_id
    ...
    LIMIT $2 OFFSET $3
  ', order_clause)
  USING place_id_param, page_limit, page_offset, requesting_user_id;
END;
$function$;