-- Update the get_restaurant_community_stats function to include user notes and captions
...
BEGIN
    ...
    WITH combined_ratings AS (
        ...
        WHERE r.place_id = place_id_param 
          ...
          AND (
            ...
            OR EXISTS (
                ...
                WHERE (f.user_id = requesting_user_id AND f.friend_id = r.user_id)
                   OR (f.friend_id = requesting_user_id AND f.user_id = r.user_id)
            )
          )
        ...
        WHERE ur.place_id = place_id_param 
          ...
          AND (
            ...
            OR EXISTS (
                SELECT 1 FROM friends f 
                WHERE (f.user_id = requesting_user_id AND f.friend_id = ur.user_id)
                   OR (f.friend_id = requesting_user_id AND f.user_id = ur.user_id)
            )
          )
    ),
    rating_stats AS (
