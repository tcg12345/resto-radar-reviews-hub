-- Check and fix other functions that might have the same table alias issue
-- Looking at build_friend_profile_cache which might also have the issue
...
BEGIN
  ...
  -- Get restaurants (limited for security) with proper alias
  SELECT jsonb_agg(
    jsonb_build_object(
      'id', rest.id,
      'name', rest.name,
  ...
  FROM (
    ...
    WHERE rest_sub.user_id = target_user_id AND rest_sub.is_wishlist = false
    ORDER BY COALESCE(rest_sub.date_visited, rest_sub.created_at) DESC
    LIMIT 100 -- Limit for security and performance
  ) rest;
  
  -- Get wishlist items (limited) with proper alias
  SELECT jsonb_agg(
    jsonb_build_object(
      'id', wish.id,
      'name', wish.name,
  ...
  FROM (
    ...
    WHERE wish_sub.user_id = target_user_id AND wish_sub.is_wishlist = true
  ...
  -- Get recent activity (limited to 10) with proper alias
  SELECT jsonb_agg(
    jsonb_build_object(
      'id', recent.id,
      'name', recent.name,
