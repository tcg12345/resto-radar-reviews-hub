-- Create indexes for better performance on restaurants table
CREATE INDEX IF NOT EXISTS idx_restaurants_user_wishlist_rating ON public.restaurants(user_id, is_wishlist, rating);
CREATE INDEX IF NOT EXISTS idx_restaurants_user_date_visited ON public.restaurants(user_id, date_visited DESC NULLS LAST);
CREATE INDEX IF NOT EXISTS idx_restaurants_user_created_at ON public.restaurants(user_id, created_at DESC);
...
-- Update the friend profile pagination function to be more efficient
...
BEGIN
  ...
  -- Check cache first for total counts
  SELECT restaurant_count, wishlist_count 
  INTO total_restaurants, total_wishlist
  FROM public.friend_profile_cache 
  WHERE user_id = target_user_id;
  
  -- If no cache, get counts directly
  IF total_restaurants IS NULL THEN
    SELECT 
  ...
  -- Get paginated data efficiently with optimized photo handling
  RETURN QUERY
  WITH stats AS (
    SELECT 
      ...
      (
        SELECT COUNT(*)::bigint 
        FROM public.restaurants 
        WHERE user_id = target_user_id AND michelin_stars > 0 AND is_wishlist = false
      ) as michelin_count
  ),
    ...
    SELECT jsonb_agg(
      jsonb_build_object(
        ...
        'photos', CASE 
          ...
          ELSE ARRAY[]::text[]
        END, -- Only first photo for performance
        'latitude', r.latitude,
